...
import com.tugas1.service.FakultasServiceDatabase;
import com.tugas1.service.MahasiswaServiceDatabase;
import com.tugas1.service.ProgramStudiServiceDatabase;
import com.tugas1.service.UniversitasServiceDatabase;
...
public class MahasiswaController {
	...
	public String viewData(Model model, @RequestParam(value = "npm") String npm) {
		...
		if (npm == null) {
			model.addAttribute("ErrorMessage", "NPM tidak boleh kosong!");
			return "error";
		} else if (!npm.matches("[0-9]+")) {
			model.addAttribute("ErrorMessage", String.format("NPM salah!", npm));
			return "error";
		}
		
		MahasiswaModel mahasiswa = mahasiswaDAO.selectMahasiswaByNPM(npm);
		if (mahasiswa == null) {
			model.addAttribute("ErrorMessage", String.format("Mahasiswa dengan NPM %s tidak ditemukan!", npm));
			return "error";
		}
